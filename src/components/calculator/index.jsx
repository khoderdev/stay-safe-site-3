...
// //   const handlePacksPerDayChange = (e) => setPacksPerDay(e.target.value);
// //   const handleYearsSmokedChange = (e) => setYearsSmoked(e.target.value);
...
// //           {smoker && (
// //             <div className='flex sm:flex-col gap-2 justify- pl-'>
...
// //                     (each pack equal to 20 cigarettes)
// //                   </span>
// //                 </label>
// //                 <input
// //                   type="number"
...
const PackYearsCalculator = () => {
  ...
  useEffect(() => {
    // Reset result if any input is invalid
    if (!smoker || packsPerDay === '' || yearsSmoked === '') {
      setResult(null);
    } else {
      calculatePackYears();
    ...
    if (!isNaN(ageNum) && gender) {
      ...
      // Clear screening messages when age or gender is not valid
      setMonthlyScreening([]);
      setYearlyScreening([]);
      setOnceAYear([]);
      setOtherScreening([]);
  ...
  //       calculatePackYears();
  //     } else {
  //       setResult(null); // Clear the result if inputs are not valid
  //     }
