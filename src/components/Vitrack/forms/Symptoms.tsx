      ...
      <div className='col-span-full'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
          {symptomsList.map((symptom, index) => (
            <label
              key={index}
              className='flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 hover:bg-gray-200 dark:hover:bg-dark cursor-pointer'
            >
              {/* Custom Checkbox */}
                ...
                <div
                  className={`w-5 h-5 flex items-center justify-center border-2 rounded-md transition-all duration-200 ${
                    symptoms.includes(symptom)
                      ? 'bg-pink border-pink'
                      : 'bg-white-bg dark:bg-dark border-gray-400 dark:border-gray-600'
                  }`}
                >
                  {/* Checkmark Icon */}
                  {symptoms.includes(symptom) && (
